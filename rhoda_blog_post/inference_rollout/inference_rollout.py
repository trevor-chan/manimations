...
MODEL_VISUAL_INSET = 0.03  # Amount to shrink on each side (total shrink = 2x this value)

# Vertical positioning - computed for consistent gaps
# Elements (top to bottom): video model, video pred, action model, vision history/action pred (same row)
...
# Action pred and vision history share the same row
_shared_row_top = _action_model_bottom - VERTICAL_GAP
_shared_row_bottom = _shared_row_top - TRACK_HEIGHT

# Total height and centering offset
_total_height = _video_model_top - _shared_row_bottom
...
# Action output and vision history are on the same horizontal level
ACTION_Y = (_shared_row_top + _shared_row_bottom) / 2 + _center_offset
VISION_HISTORY_Y = ACTION_Y  # Same row
...
class ModelContainer(VGroup):
    """Larger rounded rectangle container that encompasses both video and action models."""
    
    def __init__(self, fill_color=FILL_LIGHT_GRAY, **kwargs):
        super().__init__(**kwargs)
        
        # Calculate container dimensions
        # Spans full chunk width horizontally (hugs left of video model to right of action model)
        container_width = CHUNK_WIDTH
        
        # Spans from top of video model to bottom of action model vertically
        # No padding - exactly match the model bounds
        container_height = (VIDEO_MODEL_Y + MODEL_HEIGHT/2) - (ACTION_MODEL_Y - MODEL_HEIGHT/2)
        
        # Center Y position
        container_center_y = (VIDEO_MODEL_Y + ACTION_MODEL_Y) / 2
        
        self.box = RoundedRectangle(
            width=container_width,
            height=container_height,
            corner_radius=0.12,  # Match ModelIndicator corner radius
            stroke_width=2,
            stroke_color=fill_color,  # Match stroke to fill color
        ...
        self._center_y = container_center_y
...
class InferenceRollout(Scene):
    def construct(self):
        ...
        # Model container (outer box around both models)
        ...
        # =====================================================================
        # TEXT LABELS (Phase 1 - fade in/out with components)
        # =====================================================================
        
        # Use large font size and scale down to avoid kerning issues
        label_font_size = 96
        label_scale = 0.18  # Scale factor to achieve ~18pt equivalent
        label_margin = 0.15  # Gap between label and component
        
        # Video History label - positioned BELOW the video history block
        vision_history_label = Text(
            "video history",
            font=LABEL_FONT,
            font_size=label_font_size,
            color=TEXT_DARK
        ).scale(label_scale)
        vision_history_label.move_to([-6.5 + CHUNK_WIDTH, VISION_HISTORY_Y - TRACK_HEIGHT/2 - 0.2, 0])  # Below history block
        vision_history_label.set_opacity(0)  # Start invisible
        vision_history_label.set_z_index(50)  # Ensure it's above the vision history bar
        
        # Video Model label - positioned to the LEFT of video model, colored to match
        video_model_label = Text(
            "video model",
            font=LABEL_FONT,
            font_size=label_font_size,
            color=VISION_COLOR,
        ...
        # Position: right edge of label touches left edge of video model (minus margin)
        video_model_label_x = video_model_x - VIDEO_MODEL_WIDTH/2 - label_margin - video_model_label.get_width()/2
        video_model_label.move_to([video_model_label_x, VIDEO_MODEL_Y, 0])
        video_model_label.set_opacity(0)  # Start invisible
        
        # Action Model label - positioned to the RIGHT of action model, colored to match
        action_model_label = Text(
            "action model",
            font=LABEL_FONT,
            font_size=label_font_size,
            color=ACTION_COLOR,
        ...
        # Position: left edge of label touches right edge of action model (plus margin)
        action_model_label_x = action_model_x + ACTION_MODEL_WIDTH/2 + label_margin + action_model_label.get_width()/2
        action_model_label.move_to([action_model_label_x, ACTION_MODEL_Y, 0])
        action_model_label.set_opacity(0)  # Start invisible
        
        # Future Video Prediction label - positioned BELOW the video prediction block
        video_pred_label = Text(
            "future video prediction",
            font=LABEL_FONT,
            font_size=label_font_size,
            color=TEXT_DARK
        ).scale(label_scale)
        # Will be repositioned when video prediction appears (centered below prediction)
        video_pred_label.move_to([0, VIDEO_PRED_Y - TRACK_HEIGHT/2 - 0.2, 0])  # Placeholder position, below block
        video_pred_label.set_opacity(0)  # Start invisible
        
        # Queued Actions label - positioned BELOW the action block
        action_pred_label = Text(
            "queued actions",
            font=LABEL_FONT,
            font_size=label_font_size,
            color=TEXT_DARK
        ).scale(label_scale)
        # Will be repositioned when actions appear (centered below action area)
        action_pred_label.move_to([0, ACTION_Y - TRACK_HEIGHT/2 - 0.2, 0])  # Below action block
        action_pred_label.set_opacity(0)  # Start invisible
        
        # =====================================================================
        # ADD ELEMENTS TO SCENE WITH FADE IN
        # =====================================================================
        
        # Group visual elements for fade in (labels fade in separately with their components)
        all_initial_elements = VGroup(
            chunk_markers,
            timebar_group,
            vision_history,
            model_container,
        ...
        # Add labels to scene (invisible initially)
        self.add(vision_history_label, video_model_label, action_model_label, 
                 video_pred_label, action_pred_label)
        
        # Fade in visual elements (1 second)
        self.play(FadeIn(all_initial_elements), run_time=1)
        
        # Restore z-index after fade (grouping may affect it)
        timebar_group.set_z_index(100)
        
        # Fade in labels for initial components
        self.play(
            vision_history_label.animate.set_opacity(1),
            video_model_label.animate.set_opacity(1),
            action_model_label.animate.set_opacity(1),
            run_time=0.5
        )
        ...
        # Fade out initial labels before animation continues
        self.play(
            vision_history_label.animate.set_opacity(0),
            video_model_label.animate.set_opacity(0),
            action_model_label.animate.set_opacity(0),
            run_time=0.4
        )
        ...
        sliding_elements = [video_model, action_model, model_container, chunk_markers]
        ...
        # Position video prediction label centered BELOW the prediction block
        video_pred_label.move_to([video_pred_final_x, VIDEO_PRED_Y - TRACK_HEIGHT/2 - 0.2, 0])
        
        # Fade in the video prediction label
        self.play(video_pred_label.animate.set_opacity(1), run_time=0.3)
        
        self.wait(1.0)
        
        # Fade out the video prediction label
        self.play(video_pred_label.animate.set_opacity(0), run_time=0.3)
        ...
        sliding_elements_2 = [video_model, action_model, model_container, chunk_markers, video_prediction]
        ...
        # Position action prediction label centered BELOW the action chips
        action_chips_center_x = (action_chips[0].get_center()[0] + action_chips[-1].get_center()[0]) / 2
        action_pred_label.move_to([action_chips_center_x, ACTION_Y - TRACK_HEIGHT/2 - 0.2, 0])
        
        # Fade in the action prediction label
        self.play(action_pred_label.animate.set_opacity(1), run_time=0.3)
        
        self.wait(1.0)
        
        # Fade out the action prediction label
        self.play(action_pred_label.animate.set_opacity(0), run_time=0.3)
        ...
        caption_scale = 0.16  # Scale factor to achieve ~16pt equivalent
        
        phase2_caption = Text(
            "predicting multiple chunks into the future enables queuing actions to run during model computation, ensuring no gap in control",
            font=LABEL_FONT,
            font_size=caption_font_size,
            color=TEXT_DARK
        ).scale(caption_scale)
        phase2_caption.move_to([0, CHUNK_MARKER_BOTTOM - 0.6, 0])  # Below the timeline
        phase2_caption.set_opacity(0)  # Start invisible
        self.add(phase2_caption)  # Add to scene but don't show yet
        ...
        # Get model container position before removing
        phase1_model_container_pos = model_container.get_center()
        ...
        loop_video_model.set_opacity(0)  # Start invisible
        ...
        loop_action_model.set_opacity(0)  # Start invisible
        self.add(loop_action_model)
        
        # Fade out old models and container, fade in new ones simultaneously
        self.play(
            video_model.animate.set_opacity(0),
            action_model.animate.set_opacity(0),
            model_container.animate.set_opacity(0),
            loop_video_model.animate.set_opacity(1),
            loop_action_model.animate.set_opacity(1),
            loop_model_container.animate.set_opacity(1),
            run_time=0.5
        )
        
        # Remove old models and container
        ...
        for loop_iteration in range(3):  # Run loop 3 times
            ...
            # Fade in phase 2 caption during first iteration
            if loop_iteration == 0:
                self.play(phase2_caption.animate.set_opacity(1), run_time=0.5)
            ...
            new_video_model.set_opacity(0)  # Start invisible
            ...
            new_action_model.set_opacity(0)  # Start invisible
            ...
            def make_model_fade_out(fade_start):
                def updater(mob):
                    ...
                    mob.set_opacity(1 - fade_progress)
                return updater
            ...
            loop_model_container.add_updater(make_model_fade_out(model_fade_start_t))
            ...
            new_model_container.add_updater(make_model_fade_in(model_fade_start_t))
            
            # All elements that slide left continuously
            sliding_elements_loop = [
                loop_video_model, loop_action_model, loop_model_container, loop_chunk_markers, 
            ...
            new_model_container_initial_x = new_model_container.get_center()[0]
            ...
            # Add slide updaters to new models and container
            new_model_container.add_updater(make_continuous_slide_updater(new_model_container_initial_x))
            ...
            # Swap to new models and container
            ...
            new_model_container.clear_updaters()
            
            self.remove(loop_video_model, loop_action_model, loop_model_container)
            new_video_model.set_opacity(1)
            new_action_model.set_opacity(1)
            new_model_container.set_opacity(1)
            
            # Reset new models and container to correct position for next iteration
            new_model_container.move_to([CHUNK_WIDTH / 2, new_model_container.get_center_y(), 0])
            new_video_model.move_to([VIDEO_MODEL_WIDTH / 2, VIDEO_MODEL_Y, 0])
            new_action_model.move_to([VIDEO_MODEL_WIDTH / 2 + VIDEO_MODEL_WIDTH/2 + MODEL_GAP + ACTION_MODEL_WIDTH/2, ACTION_MODEL_Y, 0])
            
            loop_model_container = new_model_container
        ...
        # Transition captions: fade out Phase 2, fade in Phase 3
        # Use large font size and scale down to avoid kerning issues
        phase3_caption = Text(
            "realtime inference requires assessing history, predicting the future, and computing new actions many times per second",
            font=LABEL_FONT,
            font_size=caption_font_size,
            color=TEXT_DARK
        ).scale(caption_scale)
        phase3_caption.move_to([0, CHUNK_MARKER_BOTTOM - 0.6, 0])  # Same position as phase 2 caption
        phase3_caption.set_opacity(0)
        self.add(phase3_caption)
        
        # Fade out phase 2 caption (phase 3 caption will fade in during the loop)
        self.play(FadeOut(phase2_caption), run_time=0.5)
        
        # Pause between phases 2 and 3
        self.wait(1)
        
        fast_loop_time = 0.23  # 4 chunks per second
        ...
        condensed_container = RoundedRectangle(
            ...
            corner_radius=0.12,
            stroke_width=2,
            stroke_color=FILL_LIGHT_GRAY,
            fill_color=FILL_LIGHT_GRAY,
            fill_opacity=1.0
        )
        ...
        for loop_iteration in range(20):  # More iterations at fast speed
            ...
            # Fade in phase 3 caption during first iteration
            if loop_iteration == 0:
                self.play(phase3_caption.animate.set_opacity(1), run_time=0.5)
            ...
            # Create new condensed container for next chunk
            new_model_container = RoundedRectangle(
                width=CHUNK_WIDTH,
                height=MODEL_HEIGHT,
                corner_radius=0.12,
                stroke_width=2,
            ...
            new_video_model.set_opacity(0)
            
            new_action_model = ModelIndicator(width=ACTION_MODEL_WIDTH, color=ACTION_COLOR)
            new_action_model.move_to([VIDEO_MODEL_WIDTH + ACTION_MODEL_WIDTH / 2, condensed_model_y, 0])
            new_action_model.set_opacity(0)
            ...
            new_model_container.add_updater(make_slide_p3(new_model_container_x))
            ...
            loop_model_container.add_updater(make_model_fade_out_p3(model_fade_start))
            ...
            new_model_container.add_updater(make_model_fade_in_p3(model_fade_start))
